# Deploying golf_pool to Render and Connecting a Green Geeks Domain
...
- GitHub repo with the golf_pool app
...
2. Connect your GitHub account if needed, then select the **golf_pool** repository.
...
4. Add your **RAILS_MASTER_KEY**:
   - Open the **golf-pool-web** service → **Environment**.
...
6. Save. Render will build and deploy. The app will be available at `https://golf-pool-web.onrender.com` (or the URL shown in the dashboard).
...
5. Create Web Service. After deploy, note the service URL (e.g. `https://golf-pool-web.onrender.com`).

---

## 2. Add Custom Domain in Render

1. Open your **Web Service** (e.g. golf-pool-web) on Render.
...
4. Render will show the DNS records you need. You’ll use one of:
   - **CNAME** for `www` (or subdomain) → e.g. `golf-pool-web.onrender.com`
...
3. Add the records Render showed:
   ...
   **If using `www.yourdomain.com`:**
   ...
   - Target: your Render hostname, e.g. `golf-pool-web.onrender.com`
   - TTL: 300 (or 3600)
...
- [ ] App deploys and responds at the Render URL (e.g. `https://golf-pool-web.onrender.com`).
